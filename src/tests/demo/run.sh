...
WORKING_DIR=$(cd "$SCRIPT_DIR" && pnpm exec tsx -e "import { testSetup } from './setup.js'; console.log(testSetup())" 2>&1 | tail -1)
...
cd "$WORKING_DIR"
"$REPO_DIR/node_modules/.bin/tsx" "$REPO_DIR/src/index.ts"
